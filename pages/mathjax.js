...
export default function App() {
  ...
  const kebaa = 'x^2 + 2x = 10 = \\cfrac{2+4}{3} \\text{some text}'

  return (
    <MathJaxContext config={config}>
      {/* Use this first and then later get rid of your own settingss */}
      <MathJax >{`\\(${kebaa}\\)`}</MathJax>
      
      
      
      <MathJax inline>{"\\(5x * 10 \\approx 42\\)"}</MathJax>
      <MathJax inline>{"\\(5x * 10 \\approx 42\\)"}</MathJax>
      <MathJax text='kkkkx^2'></MathJax>
      ...
      <MathJax hideUntilTypeset={"first"}>
        <div>
          <div>
            ...
            <span>{`($10 / 3 x^3 \\approx 3.33$)`}</span>
          </div>
          <div>
